...
class postmarkService extends NotificationService {
  ...
  async sendNotification(event, eventData, attachmentGenerator) {
    try {
      ...
      if (typeof object === "undefined" || typeof action === "undefined" || this.options_[object] === undefined || this.options_[object][action] === undefined) return false;
    } catch (err) {
      return false;
    }
    ...
    if (data.locale && typeof templateId === "object") templateId = templateId[data.locale] || Object.values(templateId)[0]; // Fallback to first template if locale is not found
    ...
    return await this.client_.sendMail(sendOptions).then(() => ({
      to: sendOptions.To,
      status: 'sent',
      data: sendOptions
    })).catch(error => {
  ...
  normalizeThumbUrl_(url) {
    if (!url) return null;else if (url.startsWith("http")) return url;else if (url.startsWith("//")) return `https:${url}`;
    return url;
  }
